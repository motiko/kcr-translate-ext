...

{
  ...
  function observePageEvents() {
    const fullPageTranslationObserver = new MutationObserver(function (event) {
      const newValue = !!document.querySelector("html.translated-ltr, head.translated-rtl, ya-tr-span, *[_msttexthash]");
    ...
    const interval = setInterval(() => {
      const locationDataContainer = document.getElementById(kindleIframeId)
        ?.contentWindow?.document?.getElementById('kindleReader_locationPopup_labelDiv');
        ...
        if (text) {
          ...
          if (settings.name === 'google-ext') {
            ...
            if (!isFullPageTranslationMode) {
              ...
              detectedTextContainer.dispatchEvent(new MouseEvent('mouseup', {
                view: window,
                bubbles: true,
                cancelable: true,
  ...
  function handlePageTranslation() {
    ...
    if (isFullPageTranslationMode && !isPageTranslated) {
      ...
      const selectedAreas = interactionLayer.querySelectorAll('.kg-client-dictionary');
      translateSelected(selectedAreas);
